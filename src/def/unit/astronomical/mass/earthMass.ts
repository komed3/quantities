import type { UnitDef, UnitRef } from '../../../../types/def/unit';
import { Format, Lang, UnitStatus, UnitSystem } from '../../../../types/dict';
import { MassDim } from '../../../../types/dimension';

import { gram } from '../../si/base/gram';

export const earthMass = 'mEarth' as UnitRef< MassDim, 'mEarth' >;

export default ( {
  id: earthMass,
  dim: MassDim,
  structure: [],
  conversion: {
    base: gram,
    factor: 5.9722e27
  },
  prefixable: false,
  aliases: [
    'M🜨',
    'earth mass',
    'earth masses'
  ],
  context: {
    system: [
      UnitSystem.ASTRO
    ],
    status: UnitStatus.ACTIVE
  },
  meta: {
    symbol: [ {
      id: 'mEarth',
      canonical: true,
      format: {
        [ Format.PLAIN ]: 'earth mass',
        [ Format.UNICODE ]: 'M🜨',
        [ Format.LATEX ]: '\\mathrm{M_{\\oplus}}'
      }
    }, {
      id: 'ME',
      format: {
        [ Format.PLAIN ]: 'ME',
        [ Format.LATEX ]: '\\mathrm{M_{E}}'
      }
    } ],
    name: {
      [ Lang.EN ]: [ 'earth mass', 'earth masses' ],
      [ Lang.DE ]: [ 'Erdmasse', 'Erdmassen' ]
    },
    description: {
      [ Lang.EN ]: 'astronomical unit of mass, defined as the mass of the Earth, used as a standard unit of measurement in astronomy',
      [ Lang.DE ]: 'astronomische Masseneinheit, definiert als die Masse der Erde, verwendet als Standardmaßeinheit in der Astronomie'
    }
  }
} ) as const satisfies UnitDef< MassDim, typeof earthMass >;
